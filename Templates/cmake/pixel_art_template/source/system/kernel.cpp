...
bool Kernel::Awake() {
    ...
    bool vsync = false;
        ...
        switch (shared_data->game_options.screen_mode) {
            case 0:
...
void Kernel::StTitle() {
    ...
    if (title->Start()) {
        ...
        switch ((r & BITMASK_DATA_LOW)) {
            case TITLE_FLAG.new_game:               // Opcion "NEW GAME"
            case TITLE_FLAG.continue_game:
                next_state = STATE.game;
                break;
            case TITLE_FLAG.options:                // Opcion "OPTIONS"
                next_state = STATE.options;
                break;
            case TITLE_FLAG.exit:                   // Opcion "EXIT"
                next_state = STATE.exit;
                break;
            default:    // En caso de estado no definido, sal
                next_state = STATE.exit;
