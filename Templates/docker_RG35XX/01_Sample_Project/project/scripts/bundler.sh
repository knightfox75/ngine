...
# 4. libz, libexpat, libuuid: Core embedded OS libraries (False dependencies).
# ------------------------------------------------------------------------------
EXCLUDE_REGEX="^(libSDL2|libc\.so|libstdc\+\+|libgcc_s|libm\.so|libpthread|libdl|librt|ld-linux|libresolv|libnsl|libutil|libcrypt|libX11|libxcb|libXau|libXdmcp|libGL|libdbus|libasound|libgbm|libdrm|libudev|libz\.so|libexpat\.so|libuuid\.so)"
