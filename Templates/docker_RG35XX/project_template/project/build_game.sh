...
# Robust Pause Function (Ensures terminal persistence in GUI launchers)
pause_script() {
    echo -e "\n${BLUE}Press any key to close...${NC}"
    # Redirecting input from /dev/tty to ensure keypress capture
...
# --- DOCKER EXECUTION ---
# We pass PROJECT_NAME and PROJECT_VERSION as explicit arguments to the internal script
docker run --rm \
    --user $(id -u):$(id -g) \
    -v "$(pwd):/workspace" \
    $DOCKER_IMAGE \
