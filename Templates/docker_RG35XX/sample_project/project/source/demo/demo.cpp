...
bool Demo::Awake() {
    ...
    if (!ngn->graphics->Init(WINDOW_TITLE, SCR_WIDTH, SCR_HEIGHT, SCR_MODE, BILINEAR_FILTER, VSYNC)) return false;
    ngn->graphics->Update();
    ...
    ngn->graphics->SetMode(SCR_MODE);
    ngn->graphics->Update();

    // Usa el archivo empaquetado de datos
...
int8_t Demo::Run() {
    ...
    while (loop < 0) {
        ...
        if (ngn->system->quit) {    // Si se pulsa la [X] de la ventana
            loop = 0;
        } else if (ngn->input->controller[0].button[XBOX_BUTTON_X].down) {    // Si se pulsa el boton X
            loop = 1;
        }
...
bool Demo::Load() {
    ...
    bg_sound = ngn->sound->OpenMusic("data/seaside.ogg");
    if (!bg_sound) return false;
...
void Demo::MoveAim() {
    ...
    // Control por la cruceta
    if (ngn->input->controller[0].pov_up.held) speed.y = -MOVE_SPEED;
    if (ngn->input->controller[0].pov_down.held) speed.y = MOVE_SPEED;
    if (ngn->input->controller[0].pov_left.held) speed.x = -MOVE_SPEED;
    if (ngn->input->controller[0].pov_right.held) speed.x = MOVE_SPEED;
