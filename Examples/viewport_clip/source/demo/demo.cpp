...
/*** Defines ***/
#define WINDOW_TITLE "N'GINE viewport clip."            // Titulo de la ventana
...
void Demo::Clip() {
    ...
    int x, y, w, h;
    int _x = std::abs((SCR_WIDTH / 2) - ngn->input->mouse.x);
    int _y = std::abs((SCR_HEIGHT / 2) - ngn->input->mouse.y);
    x = ((SCR_WIDTH / 2) - _x);
    y = ((SCR_HEIGHT / 2) - _y);
    w = (_x * 2);
    h = (_y * 2);
